...
wget https://raw.githubusercontent.com/nadinsylaa/Magic-Script/main/telegram
chmod +x telegram
...
mv "$(echo Styrofoam-*.zip)" "$KERNEL_DIR"
cd $KERNEL_DIR
...
GITHUB_URL="https://github.com/nadinsylaa/Styrofoam-SDM439/commits/"
COMMIT=$(git log --pretty=format:'%h: %s' -1)

# Get Script Source
./telegram -f "$(echo -e Styrofoam-*.zip)" "$(echo ⚒️  [*BUILDING*] ⚒️  ️$'\n' HEAD MESSAGE:$'\n' $COMMIT $'\n' COMMIT URL: $'\n' ${GITHUB_URL}${HEAD_COMMIT} $'\n' DATE: $'\n' $KBUILD_BUILD_TIMESTAMP $'\n' BUILD USING: $'\n' $CPU $'\n' CC AUTHOR: $'\n' @NadinAlissa $'\n' DURATION: $'\n' $DURATION Seconds $'\n' ⚒️  [*COMPLETE*] ⚒️  )"
rm "$(echo Styrofoam-*.zip)"
rm telegram
